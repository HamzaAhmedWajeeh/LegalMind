...
A Retrieval-Augmented Generation (RAG) system for querying large legal document sets, with mandatory source citations on every answer and an "I don't know" fallback when retrieval comes back thin. Faithfulness is scored on every response and gated in CI.
...
```bash
git clone https://github.com/HamzaAhmedWajeeh/LegalMind.git
cd LegalMind
cp .env.sample .env
...
Hybrid retrieval (Qdrant dense + BM25, fused with RRF) reranked by Cohere Rerank v3 reaches **0.91 recall@10** on the golden dataset generated by the Adversarial Lawyer Agent.
...
- The faithfulness gate raises the floor on grounded answers; it does not eliminate hallucination. A response can score above 0.9 and still misread a clause.
- Tested on a small sample corpus. The architecture is built for tens of thousands of documents, but retrieval quality at that scale has not been measured.
- Citation validation checks that a cited chunk exists and is relevant. It does not verify that the legal proposition is correctly stated.
